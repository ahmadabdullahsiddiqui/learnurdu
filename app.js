...
function pronCard(){
  ...
  else status='<div class="pstat ok">Ready — built-in offline audio ✓ Works on every device, including iPhone &amp; iPad. No internet needed.</div>';
...
/* Offline "check": compare the drawing to a mask of the target glyph. Not
   handwriting recognition — it scores how well the trace covers the letter. */
function checkWrite(){
  ...
  drawGlyph(octx,writeText,writeDim.w,writeDim.h,'#000',24);   /* thickened target */
  var mask=octx.getImageData(0,0,W,H).data;
  var maskTotal=0,userTotal=0,hit=0,stray=0;
  for(var i=3;i<mask.length;i+=4){ var mOn=mask[i]>20,uOn=user[i]>20; if(mOn)maskTotal++; if(uOn){userTotal++; if(mOn)hit++; else stray++;} }
  if(!maskTotal){el.className='';return;}
  if(userTotal<maskTotal*0.08){ el.className='fb fb-no'; el.textContent=de?'Zeichne zuerst den Buchstaben ✍️':'Draw the letter first ✍️'; return; }
  var coverage=hit/maskTotal, strayFrac=userTotal?stray/userTotal:1, pct=Math.round(coverage*100);
  if(coverage>=0.5&&strayFrac<=0.6){ el.className='fb fb-ok'; el.textContent=(de?'Super geschrieben! ✓ ':'Great writing! ✓ ')+pct+'%'; try{celebrate();}catch(_){} }
  else if(strayFrac>0.6){ el.className='fb fb-no'; el.textContent=de?'Bleib auf den Linien 🤔':'Try to stay on the lines 🤔'; }
  else { el.className='fb fb-no'; el.textContent=(de?'Fast! Zieh den ganzen Buchstaben nach 💪 ':'Almost! Trace the whole letter 💪 ')+pct+'%'; }
}
