...
function viewWrite(){
  ...
  return '<div class="stack">'+
    ...
    '<div class="card pad" style="text-align:center"><div class="tl">'+esc(it.tl)+'</div><div class="muted">'+esc(gloss({en:it.name}))+'</div>'+
      spkBtn(it.ur)+'</div>'+
    '<div class="writepad">'+
      '<div class="writeguide ur'+(writeState.guide?'':' off')+'" id="writeGuide">'+esc(it.ur)+'</div>'+
...
var wcanvas=null,wctx=null,wdrawing=false;
function initWrite(){
  if(!writeState)return;
  var c=document.getElementById('writeCanvas'); if(!c)return;
  var ctx; try{ctx=c.getContext('2d');}catch(e){ctx=null;} if(!ctx)return;
  wcanvas=c; wctx=ctx;
  function size(){
    var r=c.getBoundingClientRect(), dpr=window.devicePixelRatio||1;
    c.width=Math.max(1,Math.round(r.width*dpr)); c.height=Math.max(1,Math.round(r.height*dpr));
    ctx.setTransform(dpr,0,0,dpr,0,0);
    ctx.lineCap='round';ctx.lineJoin='round';ctx.lineWidth=8;ctx.strokeStyle='#0e7d3e';
  }
  size();
  var pos=function(ev){var r=c.getBoundingClientRect();return {x:(ev.clientX-r.left),y:(ev.clientY-r.top)};};
  c.addEventListener('pointerdown',function(ev){wdrawing=true;c.setPointerCapture&&c.setPointerCapture(ev.pointerId);var p=pos(ev);wctx.beginPath();wctx.moveTo(p.x,p.y);wctx.lineTo(p.x+0.1,p.y+0.1);wctx.stroke();ev.preventDefault();});
...
function clearWrite(){ if(wctx&&wcanvas)wctx.clearRect(0,0,wcanvas.width,wcanvas.height); }
...
function onClick(e){
  ...
  if((el=t.closest('[data-writeguide]'))){writeState.guide=!writeState.guide;render();return;}
