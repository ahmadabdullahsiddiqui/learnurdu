...
/* iOS/Safari only starts the speech engine inside a user gesture; warm it
   up once on the first tap so the first word is not swallowed. */
...
function speak(text){
  ...
  try{
    ...
    if(v)u.voice=v;
      ...
      S.hintShown=true;save();
      toast('No Urdu voice on this device yet. Install one for spoken pronunciation — Android: Settings › System › Languages & input › Text-to-speech; iPhone: Settings › Accessibility › Spoken Content › Voices.');
    }
...
function pronCard(){
  ...
  if(!vs.length)
    h+='<p class="tiny" style="margin:0">No Urdu voice is installed yet. Tap <b>Test</b> to try the system default; if it is silent, install a voice — Android: Settings › System › Languages &amp; input › Text-to-speech; iPhone: Settings › Accessibility › Spoken Content › Voices.</p>';
  return h+'</div>';
}
